...
# 2つの日付の差から経過期間(y年mか月とd日)を求めたい

　ある日付からの経過日数を「何年何か月と何日」の形式で求めたい。総年数や総月数、総日数であれば`lubridate`パッケージの関数で簡単に求められるが、前月の同じ日にちからの経過日数を求めるのは(たぶん)ややこしいのでやってみる。簡単に求められるのであれば教えてほしい。\
\
...
　各期間の開始日を`"yyyy-mm-dd"`などの形式の文字列で指定して、`as_date()`で日付型の値に変換する。\
...
　計算を行う年数を`year_len`として、指定した日付`date_from`の「翌月の1日」から「`year_len`年後の当月の1日」まで1か月間隔の日付を`seq(by = "month")`で作成して、データフレームに格納する。\

　条件式や計算用に、各日付の日にちを`date_day`列、基準日の日にちを`from_day`列、基準日の前月の日にちを`pre_last_day`列としておく。日にちの値は`day()`、前月の月末の日付は`rollback()`で得られる。\

　基準日`date_from`から各日付`date`までの経過年数を`interval()`と`time_length(unit = "year")`で計算して`y`列とする。\
　同様に、総月数を`interval()`と`time_length(unit = "month")`で計算して、さらに`12`で割った余りを経過月数`m`列とする。商余は`%%`で計算できる。\
　経過日数は、`case_when()`を使って、基準日によって場合分けして計算して`d`列とする。

- 基準日の日にちが前月に存在しない(`from_day`が`pre_last_day`より大きい)または月末(`from_day`と`pre_last_day`が同じ)の場合は、基準日を月末とみなして、経過日数は`1`である。
- 基準日が経過日と同じ日にち(1日)(`from_day`が`1`)の場合は、経過日数は`0`である。
- 基準日の日にちが前月に存在する(前月の末日より前の日)(`from_day`が`pre_last_day`より小さい)または月末(`from_day`と`pre_last_day`が同じ)の場合は、前月の基準日の日から月末までの日数(`pre_last_day - from_day`)の`1`にち後である。\

　総日数は`interval()`と`time_length(unit = "day")`で計算できる。\
...
　基準日`date_from`と、計算を行う期間を年数`year_len`と日数`day_len`で指定する。\
　`date_from`から指定した期間後までの1日間隔の日付を`seq(by = "day")`を作成して、データフレームに格納する。\

　「月ごとの経過日数」のときと同様にして、各日付の日にち列`date_day`、基準日の日にち列`from_day`、基準日の前月の日にち列`pre_last_day`を作成しておく。\
　また、基準日`date_from`から各日付`date`までの経過年数`y`、経過月数`m`を計算する。\

　経過日数`d`を、「基準日」と「経過日」または「経過日の前月の末日」との大小関係によって場合分けして計算する。

- 「経過日の日にち」から「基準日の日にち」までの日数を計算する(`form_day`が`date_day`より大きい)場合は、さらに2つの場合に応じて計算を行う。
    - 基準日の日にちが前月に存在する(前月の末日より前の日)(`from_day`が`pre_last_day`より小さい)場合は、「前月の基準日」から「月末までの日数」(`pre_last_day - from_day`)と「当月の経過日」`date_day`の和である。
    - 基準日の日にちが前月に存在しない(`from_day`が`pre_last_day`より大きい)または月末(`from_day`と`pre_last_day`が同じ)の場合は、基準日を月末とみなして、経過日数は`date_day`である。
- 「基準日の日にち」から「経過日の日にち」までの日数を計算する(`form_day`が`date_day`以下の)場合は、`date_day - from_day`である。
...
　この例では、x軸をの年月、y軸を日として可視化する。`date`列から年と月部分を`format()`で文字列として取り出す。\

　ヒートマップとラベルで確認する。

```{r, fig.width=15, fig.height=10, dpi=100}
# 経過期間をヒートマップで可視化
ggplot() + 
...
　基準日の日にち(この例だと15日)における経過日数は全て0日なのを確認できる。月末の日数が異なるので、基準日の日にちより前の日にちでは経過日数が異なる。\
\


### カレンダーで確認

　同様に、カレンダー形式で確認する。(色々あって同時期にカレンダーを作ったのでやってみるだけで、特に意味はない。たぶんこっちの方が分かりにくい。)詳しくは「ggplot2でカレンダーを作成した」を参照のこと。\
\
...
# カレンダー用に経過期間データを整形
calendar_df <- date_df |> 
  dplyr::select(date, day = date_day, d, days, ymd_label) |> 
  dplyr::bind_rows(
    tibble::tibble(
      # 1日間隔の日付を作成
      date = seq(
...
　基準日以前の同月の日付を追加して、各日付が何曜日であるかと第何週であるかを計算する。基準日以前の日付は、欠損値にするか取り除く。この例ではどちらも行っている。\

　基準日を示すためのデータフレームを作成する。
